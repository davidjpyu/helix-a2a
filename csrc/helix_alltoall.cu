...
#ifndef HELIX_A2A_CHECK
#define HELIX_A2A_CHECK(cond, ...)                                 \
  do {                                                             \
    if (!(cond)) {                                                 \
...
#ifndef HELIX_A2A_CUDA_CHECK
#define HELIX_A2A_CUDA_CHECK(call)                                 \
  do {                                                             \
...
std::tuple<int, int, int> computeChannelAndGroupCount(
  ...
  static std::unordered_map<std::tuple<int, int, int>,
      ...
      cache;
  int deviceId = 0;
  HELIX_A2A_CUDA_CHECK(cudaGetDevice(&deviceId));
  ...
  HELIX_A2A_CUDA_CHECK(cudaDeviceGetAttribute(
  ...
  HELIX_A2A_CHECK(totalDynamicShmemSize <= maxDynamicShmSize,
                   "Single packed size %d exceeds limit %d", singleShmSize,
                   maxDynamicShmSize);

  if (totalDynamicShmemSize > 48 * 1024) {
    HELIX_A2A_CUDA_CHECK(cudaFuncSetAttribute(
...
static bool getEnvEnablePDL() {
  static int cached = -1;
  if (cached < 0) {
    const char* env = std::getenv("HELIX_A2A_ENABLE_PDL");
...
void launchHelixAllToAllImpl(HelixAllToAllParams const& params,
  ...
  if (params.channelCount > 0) {
    ...
    HELIX_A2A_CHECK(channelCount <= maxChannelCount,
  ...
  config.numAttrs = 1;
  config.attrs = attrs;
  HELIX_A2A_CUDA_CHECK(cudaLaunchKernelEx(&config, kernel_instance, params));
}
