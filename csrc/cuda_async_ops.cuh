...
__device__ __forceinline__ void mbarrier_init(uint64_t* addr,
                                              uint32_t const& count) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 800
  asm("mbarrier.init.shared.b64 [%0], %1;"
      :
      : "r"(__as_ptr_smem(addr)), "r"(count)
      : "memory");
#endif
}

__device__ __forceinline__ void mbarrier_expect_tx(uint64_t* addr,
...
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 800
  uint64_t state;
  asm("mbarrier.arrive.shared.b64 %0, [%1];"
      : "=l"(state)
      : "r"(__as_ptr_smem(addr))
      : "memory");
  return state;
#else
  return 0;
...
mbarrier_arrive_expect_tx(uint64_t* addr, const uint32_t txCount) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 900
  uint64_t state;
...
__device__ __forceinline__ bool mbarrier_try_wait_parity(
    uint64_t* addr, uint32_t const& phaseParity) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 900
...
__device__ __forceinline__ void ldgsts(int* dstShm, int const* srcMem,
                                       bool predGuard) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 800
  asm volatile(
      "{\n"
...
__device__ __forceinline__ void cp_async_bulk_g2s(void* dstMem,
                                                  void const* srcMem,
                                                  int copySize,
                                                  uint64_t* smemBar) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 900
...
__device__ __forceinline__ void cp_async_bulk_s2g(void* dstMem,
                                                  void const* srcMem,
                                                  int copySize) {
#if defined(__CUDACC__) && __CUDA_ARCH__ >= 900
...
__device__ __forceinline__ void smemBarWait(uint64_t* smemBar,
